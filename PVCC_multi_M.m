function [W, b, f, f_init] = PVCC_multi_M(data, truth, options);
...
for iterV = 1:v
	...
	eta(iterV) = sum(indM(iterV, :));
	
	X{iterV} = data{iterV};
	
    Gamma{iterV} = indM(iterV, :)==1;
    indM(iterV, :);
	P{iterV} = zeros(N, c);
	P{iterV}(Gamma{iterV}, :) = 1;

	E{iterV} = zeros(N, 1);
	E{iterV}(Gamma{iterV}, :) = 1;
...
for i = 1:v
	W{i} = aMb{i}*C{i}*(f.*P{i});
	b{i} = (X{i}*W{i}-(f.*P{i}))'*ones(N, 1)/eta(i);
end
